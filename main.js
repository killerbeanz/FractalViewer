...
const GRADIENT_BUFFER_BYTES = 352;
...
const fullscreenScreenshotBtn = document.getElementById('fullscreen-screenshot-btn');
...
function resizeCanvasAndBuffers() {
  ...
  if (totalPixels > maxPixels) {
    ...
    statusEl.textContent =
      'Resolution capped for this GPU.';
  }
...
async function readDoneCount() {
  if (doneReadPromise) {
    return doneReadPromise;
  }

  doneReadPromise = (async () => {
    const encoder =
    ...
    await doneReadbackBuffer.mapAsync(
      GPUMapMode.READ
    );
  ...
  try {
    return await doneReadPromise;
  } finally {
    doneReadPromise = null;
  }
}
...
async function createFullscreenScreenshot() {
  if (
  ...
  fullscreenScreenshotBtn.disabled =
    true;
  ...
  try {
    ...
    statusEl.textContent =
      'Rendering fullscreen screenshot…';
...
function drawGradientPreview() {
  const ctx =
    gradientPreview.getContext(
      '2d'
    );

  const width =
    gradientPreview.width;

  const height =
    gradientPreview.height;
...
gradientClose.addEventListener(
  'click',
  () => {
    gradientEditor.classList.remove(
      'open'
    );
  }
);
...
fullscreenScreenshotBtn.addEventListener(
  'click',
